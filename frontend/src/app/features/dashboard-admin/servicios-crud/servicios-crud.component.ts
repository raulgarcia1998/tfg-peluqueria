  ...
  template: `
    <div class="p-8">
      <div class="flex items-center justify-between mb-8">
        <div>
          <h2 class="text-3xl font-bold text-white mb-2">Servicios</h2>
          <p class="text-gray-400">Gestiona el catálogo de servicios de la peluquería.</p>
        </div>
        <button (click)="abrirModal()"
                class="flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-sm text-gray-900 transition-all hover:scale-105"
              ...
              <div>
                <div class="flex justify-between items-start mb-4">
                  ...
                  <div class="flex gap-2">
                    <button (click)="abrirModal(s)" class="p-2 rounded-lg hover:bg-white/10 text-gray-400 hover:text-white transition-all">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/>
                      </svg>
                    </button>
                    <button (click)="eliminar(s.id)" class="p-2 rounded-lg hover:bg-red-500/10 text-gray-400 hover:text-red-400 transition-all">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <h3 class="text-xl font-bold text-white mb-1">{{ s.nombre }}</h3>
                <p class="text-gray-400 text-sm mb-4 line-clamp-2">{{ s.descripcion || 'Sin descripción' }}</p>
              </div>
              
              <div class="flex items-center justify-between mt-4 pt-4 border-t" style="border-color:rgba(255,255,255,.08)">
                <div class="flex items-center gap-2">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#d4af37" stroke-width="2">
                    <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                  </svg>
                  <span class="text-gray-300 text-sm font-medium">{{ s.duracionMin }} min</span>
                </div>
                <span class="text-xl font-bold text-white">{{ s.precio }}€</span>
              </div>
            </div>
          }
        </div>
      ...
      @if (mostrarModal()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm"
          ...
          <div class="w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl"
            ...
            <div class="px-8 py-6 border-b" style="border-color:rgba(255,255,255,.08)">
              <h3 class="text-2xl font-bold text-white">{{ editando() ? 'Editar' : 'Nuevo' }} Servicio</h3>
            </div>
            ...
            <div class="p-8 space-y-5">
              <div>
                ...
                <input type="text" [(ngModel)]="form.nombre" placeholder="Corte degradado"
                       class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white outline-none focus:border-[#d4af37] transition-all">
              </div>
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-400 mb-2">Precio (€)</label>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-medium text-gray-400 mb-2">Duración (min)</label>
                  <input type="number" [(ngModel)]="form.duracionMin"
                         class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white outline-none focus:border-[#d4af37] transition-all">
                </div>
                <div class="flex items-end pb-3">
                   <label class="flex items-center gap-3 cursor-pointer">
                     <input type="checkbox" [(ngModel)]="form.activo" class="w-5 h-5 accent-[#d4af37]">
              ...
              <div>
                ...
                <textarea [(ngModel)]="form.descripcion" rows="3"
                          class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white outline-none focus:border-[#d4af37] transition-all resize-none"></textarea>
              </div>

              <div class="flex gap-4 pt-4">
                <button (click)="cerrarModal()"
                        class="flex-1 py-4 rounded-2xl font-bold text-gray-400 transition-all hover:text-white"
                        style="background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.08)">
                  Cancelar
                </button>
                <button (click)="guardar()"
                        class="flex-1 py-4 rounded-2xl font-bold text-gray-900 transition-all hover:scale-[1.02]"
                        style="background:#d4af37">
                  Guardar
                </button>
              </div>
