...
@Component({
  ...
  template: `
    <div class="p-8">
      <div class="mb-8">
        <h2 class="text-3xl font-bold text-gray-900 mb-2">Mis Citas</h2>
        <p class="text-gray-600">Gestiona tus reservas y consulta tu historial.</p>
      </div>
      ...
      @if (cargando()) {
        ...
        <div class="flex flex-col items-center justify-center py-20 text-center"
             style="background:var(--surface-1); border:1px dashed var(--border-1); border-radius: 2rem;">
          ...
          <h3 class="text-xl font-bold text-gray-900 mb-2">No tienes citas próximas</h3>
          ...
          <button routerLink="/user/reservar"
                  class="px-8 py-3 rounded-xl font-bold text-sm text-white transition-all hover:scale-105"
          ...
          @for (cita of citas(); track cita.id) {
            <div class="p-6 rounded-2xl flex flex-col sm:flex-row gap-6 relative overflow-hidden"
              ...
              <div class="flex-1">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-1">{{ cita.servicio?.nombre ?? 'Corte de pelo' }}</h3>
                <p class="text-gray-600 text-sm mb-4">Con {{ cita.empleado?.nombre ?? 'nuestro equipo' }}</p>
              ...
              <div class="flex sm:flex-col justify-end gap-3 shrink-0">
                @if (cita.estado === 'PENDIENTE' || cita.estado === 'CONFIRMADA') {
                  <button (click)="cancelarCita(cita.id)"
                          class="px-4 py-2 rounded-xl text-sm font-semibold text-red-600 transition-all hover:bg-red-500/10"
